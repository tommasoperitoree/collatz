...
@app.cell
def _(mo):
    mo.md("# 🌿 Collatz Conjecture Explorer")
    return
...
def _(max_val_dropdown, mo, n_starts_slider, np, run_button):
    ...
    return MAX_VAL, N_STARTS, anchor, collatz_length, longest_len, longest_start, starts


@app.cell
def _(MAX_VAL, N_STARTS, defaultdict, longest_len, longest_start, mo, np, starts):
        ...
        for _num, _label in zip([anchor, longest_start], [anchor_label, longest_label]):
            if _num in node_pos:
                ...
                ax.text(_x - 0.1, _y + 0.1, _label, fontsize=6, fontfamily=FONT,
                        ha="right", va="top", color=(_r, _g, _b))
